...
export default function Feed({subcat}) {
  ...
  const api=axios.create({
    baseURL:"https://harenastore.onrender.com/subcategory"
  })
  ...
  useEffect(()=>{
    console.log("the subcategory in feed 2 is ",subcat);
    let x=(page-1)*4;
    api.get(`/?cname=${subcat}&page=${page}`)
    .then(async res=>{
      if(res.status===200){
        let bd=res.data;
        items=bd;
